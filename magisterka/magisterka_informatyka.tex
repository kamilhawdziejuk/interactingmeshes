...
W niniejszej pracy skupiê siê na obiektach z przestrzeni $R^3$.

Obiekty takie w grafice komputerowej  bardzo czêsto reprezentowane s± w postaci przybli¿onej. Istniej± specjalne algorytmy triangulacji, których zadanie polega na rozbiciu bardziej z³o¿onych obiektów na siatki trójk±tów, które je pokrywaj±. W procesie tym powstaje wielok±t (wielo¶cian), który jest mo¿liwie bliskim przybli¿eniem danego obiektu. Warto w tym miejscu wspomnieæ, ¿e triangulacja coraz bardziej wspierana jest ze strony sprzêtowej i ze wzglêdów wydajno¶ciowych zajmuj± siê tym obecnie podzespo³y graficzne jeszcze w procesie renderowania. Dlatego te¿ w systemach kolizji najczê¶ciej wybieran± reprezentacj± jest siatka wielok±tów (przewa¿nie trójk±tów) reprezentuj±cych obiekt z przestrzenii. 

W zwi±zku z tym, moim naturalnym za³o¿eniem w dalszej czê¶ci pracy jest to, i¿  wszystkie analizowane obiekty s± wielo¶cianami, a ka¿dy z nich utworzony jest z:
...
Im dok³adniejsze sprawdzenia i wyniki s± potrzebne, tym wiêksza staje siê z³o¿ono¶æ ca³ego systemu zarówno pod k±tem obliczeniowym jak i pamiêciowym. Ponadto nie wszystkie reprezentacje obiektów wspieraj± wszystkie zapytania.

\section{Parametry ¶rodowiska}
Aplikacja symuluj±ca (np. CAD lub gra) sama w sobie zawiera liczne parametry, które wp³ywaj± po¶rednio b±d¼ bezpo¶rednio na system wykrywania kolizji. Wp³ywaj± na to miêdzy innymi: liczba obiektów, ich rozmiary, pozycje, mo¿liwe sposoby poruszania, zezwalanie na przenikanie, tryb ich wykrywania oraz rozwi±zywania itd.
...
W¶ród wszystkich problemów wystêpuj±cych w ¶rodowiskach 3D, te odnosz±ce siê do kolizji obarczone s± najbardziej krytycznymi ograniczeniami czasowymi. Wiêkszo¶æ ¶rodowisk z³o¿ona jest z du¿ej liczby (nierzadko skomplikowanych) obiektów, a oczekuje siê, ¿e ca³o¶æ dzia³aæ bêdzie w miarê p³ynny sposób. W zwi±zku z tym wprowadza siê pewne usprawnienia przyspieszaj±ce wykrywanie kolizji, które omówione zostan± w tym rozdziale - to podzia³y przestrzeni.

S± to struktury, które dziel±  przestrzeñ $d$-wymiarow± na pary podprzestrzeni (obiekty dziel±ce s± wtedy $(d-1)$-wymiarowymi hiperp³aszczyznami). W szczególno¶ci, scenê 3D przecinaæ mo¿emy p³aszczyznami, otrzymuj±c dwie pó³przestrzenie znajduj±ce siê po prawej i lewej jej stronie.

Struktury te zasadniczo wykorzystywane s± na dwa sposoby. Z jednej strony, na poziomie makro, mo¿emy dziêki nim redukowaæ liczbê koniecznych do przeprowadzenia testów kolizji pomiêdzy wszystkimi parami obiektów. Zauwa¿my, ¿e dla $n$ obiektów istnieje $\frac{ n (n-1)}{2}$ par, które mog± ze sob± kolidowaæ. Wprowadziwszy sensowny podzia³ przestrzeni na mniejsze komórki, mo¿emy liczbê tych par zmniejszyæ do minimum i testowaæ jedynie te, które znajduj± siê w tych samych komórkachi. Z drugiej strony, na poziomie mikro, wykorzystywane s± do redukowania liczby koniecznych do sprawdzenia przeciêæ fragmentów obiektów w poszczególnych testach kolizji. W przypadku bowiem, gdy mamy zweryfikowaæ czy dwie z³o¿one siatki ze sob± koliduj±, mo¿emy ograniczyæ testy przeciêæ ich fragmentów do tych czê¶ci podzielonej przestrzeni, w której faktycznie kolizja mo¿e dla nich wystêpowaæ\footnote{Drzewa BSP wynalezione zosta³y na pocz±tku jako struktury rozwi±zuj±ce problem reprezentowania niewidocznych miejsc w przestrzeni. Maj± ponadto du¿e zastosowanie w ray traycing'u, kontrukcji obiektów CSG czy planowaniu ¶cie¿ek robotów.}. 

\clearpage
\subsection{Kolizje na zbiorach wklês³ych}

W szczególno¶ci, przedstawionymi wy¿ej podzia³ami mogliby¶my zdekomponowaæ zbiór wklês³y na zbiory wypuk³e i zastosowaæ do wykrycia kolizji ka¿dego z nich algorytm GJK, opisany w poprzednim rozdziale. Niestety, takie podej¶cie, z dwóch powodów, nie jest najw³a¶ciwsze z mo¿liwych. Przede wszystkim zagadnienie znalezienia minimalnego podzia³u zbioru wypuk³ego na zbiory wklês³e jest problemem $NP$-trudnym. Poza tym istnieje siatka zbudowana z $n$ trójk±tów, która z³o¿ona jest z $\Omega(n^2)$ zbiorów wypuk³ych. To wszystko doprowadza najczê¶ciej do decyzji postêpowania z zagadnieniem w sposób polegaj±cy na dekompozycji obiektu na mniejsze obiekty, które mog± byæ szybciej przetestowane i niekoniecznie s± zbiorami wypuk³ymi\footnote{Ostatnimi czasy powstaje wiele innych ciekawych podej¶æ, jak np. wyniki naukowców z Texas A\&M University, dotycz±ce algorytmów przybli¿onej dekompozycji zbioru wklês³ego na zbiory wypuk³e, publikowywane na stronie: http://parasol.tamu.edu/groups/amatogroup/research/app-cd/}.

\subsection{Typy struktury BSP i jej konstrukcja}
Jedn± z najbardziej wszechstronnych metod podzia³u jest tworzenie struktury \textbf{Binary Space-Partitioning tree} (w skrócie BSP tree), któr± te¿ zaimplementowa³em w do³±czanej aplikacji. Polega ona na ko¿dorazowym rekurencyjnym podziale danej przestrzeni hiperp³aszczyznami na dwie mniejsze wypuk³e czê¶ci - zwane komórkami.  Weryfikowanie kolizji polega dalej na sprawdzaniu przecinania siê fragmentów siatek dwóch obiektów tylko wtedy, gdy oba nale¿± do tej samej komórki, co stanowi ogromne przyspieszenie.
...
- je¶li $card(S) > 1$, to korzeñ $v$ drzewa $T$ pamiêta hiperp³aszczyznê $h_v$ wraz ze zbiorem obiektów, które s± ca³kowicie w niej zawarte. Lewym dzieckiem wêz³a $v$ jest korzeñ zawieraj±cy zbiór $S^{-} :=\{h_{v}^{-} \cap s : s \in S\}$, a prawym dzieckiem wêz³a $v$ jest korzeñ zawieraj±cy zbiór $S^{+} := \{h_{v}^{+} \cap s : s \in S\}.$
\end{df}

Drzewa BSP mog± byæ podzielone na wiele sposobów: g³ównie ze wzglêdu na sposób wybierania hiperp³aszczyzny dziel±cej, ale te¿ na sposób przechowywania geometrii (w wierzcho³kach, w li¶ciach), czy ze wzglêdu na sam obiekt podzia³u - czy jest to ca³a przestrzeñ czy jeden obiekt. W szczególno¶ci, wyró¿niono trzy ich typy: $node-storing$ (przechowuj±ce w wêz³ach), $leaf-storing$ (przechowuj±ce w li¶ciach) i $solid-leaf$ (wype³nione li¶æmi). 


\textbf{Przechowuj±ce w wêz³ach.}
 W takim przypadku wszystkie ¶ciany, które nale¿± do danej p³aszczyzny podzia³u $h$ l±duj± w wê¼le drzewa, a pozosta³e s± przekazywane rekurencyjnie do poddrzew zgodnie z ich przynale¿no¶ci± do odpowiednich podprzestrzeni $h^{-}$ oraz $h^{+}$ (¶ciany, które przecinaj± siê z $h$ zostaj± podzielone na dwie czê¶ci i równie¿ przekazane do odpowiednich podrzew). Oczywi¶cie li¶cie mog± byæ puste lub zawieraæ tylko jedn± ¶cianê.
\begin{figure}[htbp]
 \centering
...
\caption[Rozmiar 20]{Wype³niona figura poprzecinana i odpowiadaj±ce jej drzewo BSP wype³nione li¶æmi.}
  \label{zrodlo}
\end{figure}
...
Jednym z wa¿niejszych zadañ hierarchii budowanego podzia³u jest utrzymywanie jak najmniejszej liczby wêz³ów w drzewie BSP. Dla siatek o $n$ trójk±tach i drzewa BSP zbudowanego metod± autopodzia³u liczba jest wêz³ów zbli¿ona jest do $O(nlogn)$, a wysoko¶æ do $O(logn)$, wiêc przeszukiwanie go jest szybkie. Niestety utworzenie struktury BSP z autopodzia³em ma z³o¿ono¶æ $O(n^2)$, dlatego te¿ jej stosowanie najczê¶ciej sprowadza siê do badania kolizji na obiektach statycznych, rzadziej poruszaj±cych siê.
Z drugiej strony utworzenie struktury BSP, w którym p³aszczyzna podzia³u jest równoleg³a do osi zajmuje ¶rednio $O(nlogn)$, a jego wysoko¶æ $O(n)$.
...
Istnieje wiele podej¶æ do wykrywania i rozwi±zywania kolizji w scenie. W poprzednich podrozdzia³ach $2.1 - 2.3$ opisa³em szczegó³owo jedynie po jednym z ka¿dego etapu testowania (wszystkie zaimplementowane s± w do³±czanej aplikacji). Wybra³em je, gdy¿ s± w pewnym sensie najbardziej wszechstronne spo¶ród pozosta³ych, przez co wybierane s± te¿ prawdopodobnie jako jedne z najczêstszych. Chcia³bym tu jednocze¶nie podkre¶liæ, ¿e tematyka kolizji stanowi poka¼ny dzia³ z szerok± gam± algorytmów z powodzeniem stosowanych w praktyce.
Przedstawiê poni¿ej pokrótce jeszcze kilka innych algorytmów, pojawiaj±cych siê w cytowanej przeze mnie literaturze.

\subsection{Grupa testów, zak³adaj±cych wypuk³o¶æ obiektów}
...
\subsection{Grupa szczegó³owych testów na kolizje}

Wiêkszo¶æ z nich polega na szczególnym podziale zbioru b±d¼ przestrzenii na mniejsze komórki (tzw. ''spatial partitioning''). Przyk³adami takiego podzia³u s±: \textbf{k-d tree}, \textbf{quadtree} czy \textbf{octree}. 

Istniej± wreszcie stosowane z powodzeniem struktury tzw. \textbf{k-DOP} opakowuj±ce dany obiekt w wielo¶cian o $k$-¶cianach, na których przeprowadzane s± przybli¿one kolizje (dla $k=6$ s± one równowa¿ne z opisanym w rozdziale $2.1$ testem na prostopad³o¶cianach).

\chapter{Metody poprawy wydajno¶ci systemów detekcji kolizji}

Istnieje kilka istotnych kwestii wydajno¶ciowych, które nie sposób pomin±æ omawiaj±c tematykê systemów detekcji kolizji. 

Przede wszystkim wiele z przedstawianych algorytmów mo¿na przyspieszyæ wykorzystuj±c \textbf{programowanie równoleg³e}. Wiêkszo¶æ z obecnie produkowanych komputerów posiada procesory wielordzeniowe. W zwi±zku z tym coraz bardziej warto jest przyspieszaæ ich dzia³anie wykorzystuj±c mo¿liwo¶ci sprzêtowe\footnote{Prawdopodobnie w przysz³o¶ci bêdzie to mia³o o wiele wiêksze znaczenie dziêki przetwarzaniu rozproszonemu, do którego dostêp bêdzie dziêki powszechnie u¿ywanej technologiii przetwarzania w chmurze, czyli tzw.''Claud computing''.}. W szczególno¶ci wszelkiego typu struktury podzia³u, w których kolizje badamy rekurencyjnie w ka¿dej z komórek przestrzenii (rozdzia³ 2.3.),  mo¿na przerobiæ na wersjê równoleg³±, by w ca³kiem sensownym czasie uzyskaæ kilkukrotne przyspieszenie.

Czêsto jeste¶my w stanie okre¶liæ, które obiekty nie bêd± podlega³y transformacjom (np. ¶ciany). Kolejnym podej¶ciem usprawniaj±cym wydajno¶æ jest \textbf{budowanie BSP dla obiektów statycznych} i przetrzymywanie ich razem z nimi w przeci±gu ca³ego czasu ich istnienia w aplikacji. W momencie sprawdzania kolizji nie istnieje wtedy potrzeba budowania tej struktury w locie (co jest najbardziej czasoch³onne), a jedynie przeprowadzenia wyszukiwania w danym drzewie.

Tak samo, mo¿emy te¿ ''przetworzyæ'' wstêpnie obiekty np. pod k±tem ich wypuk³o¶ci (je¶li s± równe swoim otoczkom wypuk³ym, to s± wypuk³e). Je¶li w przeci±gu swojego istnienia w aplikacji nie podlegaj± one deformacjom, to mo¿emy \textbf{oznaczyæ flag± obiekty, które s± wypuk³e} i pomijaæ pewne dalsze (a przy tym bardziej z³o¿one) etapy wykrywania kolizji. Przyk³adowo, obiekty, które s± oba wypuk³e mog± zatrzymaæ test swojej kolizji w momencie zatrzymania algorytmu GJK z wynikiem prawdziwym.
...
Na potrzeby przetestowania powy¿szych algorytmów napisa³em aplikacjê o nazwie ''InteractingMeshes', która jest integraln± czê¶ci± mojej pracy, stworzon± do symulacji kolizji. Aplikacja dostêpna jest pod adresem:
...
Program ''InteractingMeshes'' jest aplikacj± s³u¿±c± do prezentowania kolizji na siatkach w scenie 3D. U¿ytkownik mo¿e wstawiaæ, wybieraæ oraz usuwaæ obiekty przy pomocy panelu Obiekty w oknie Opcje:
...
\caption[Rozmiar 20]{Opcje programu (panel testów kolizji i panel obiektów).}
  \label{zrodlo}
\end{figure}

Na dan± chwilê s± to standardowe obiekty dostêpne z klasy Mesh w bibliotece Directx3D (prostopad³o¶cian, sfera, torus, czajnik). Dla ka¿dego z nich istnieje szereg parametrów (np. dla sfery s± to: d³ugo¶æ promienia, liczba równole¿ników, liczba po³udników), które go definiuj±. Dlatego te¿ operowaæ mo¿na na szerokiej gamie siatek. W kolejnych wersjach programu zamierzam dodaæ mo¿liwo¶æ importowania zewnêtrznych siatek zdefiniowanych w plikach w standardowym formacie *.X.

Ponadto w panelu ''Testy kolizji''  mo¿ena ustawiaæ rodzaj algorytmów detekcji kolizji, które maj± zostaæ zastosowane w programie do ich wykrywania. S± to poszczególne testy opisane przeze mnie szczegó³owo we wcze¶niejszych rozdzia³ach. Zaznaczenie opcji ''Test Boxes'' powoduje sprawdzanie kolizji obiektów na prostopad³o¶cianach opakowuj±cych (rozdzia³ 2.1.), opcji ''Test GJK''  sprawdzanie kolizji algorytmem Gilberta-Johnsona-Keerthi (rozdzia³ 2.2.), a opcji ''Test BSP'' sprawdzanie kolizji przy pomocy struktury podzia³u BSP (rozdzia³ 2.3.) jedn± z dwóch mo¿liwych metod. W kolejnych wersjach programu zamierzam poszerzyæ mo¿liwo¶ci aplikacji implementuj±c inne metody.

\section{Szczegó³y implementacyjne}

Projekt napisa³em w technologii .NET w jêzyku programowania C\# u¿ywaj±c ¶rodowiska Microsoft Visual Studio 2008. Do wizualizacji i operacji graficznych wykorzysta³em bibliotekê Directx w wersji 9.0. 
...
\addcontentsline{toc}{chapter}{\hspace{12pt} Zakoñczenie}

\chapter*{Zakoñczenie}

Niniejsza praca magisterska jest wynikiem moich studiów na temat systemów detekcji kolizji w ¶rodowiskach symulacyjnych. Postanowi³em ogólnie opisaæ ich za³o¿enia oraz szczegó³owo skupiæ siê na wybranych algorytmach razem z ich implementacj±, maj±c ju¿ pewne do¶wiadczenie zawodowe w tej dziedzinie. Mam nadziejê, ¿e uda³o mi siê w zgrabny sposób przedstawiæ te wyniki zarówno od strony teoretycznej jak i praktycznej poprzez do³±czenie ¼róde³ programu. 
...
7. www.geometrictools.com - zestaw u¿ytecznych kodów i algorytmów wykorzystywanych w grafice komputerowej, matematyce, fizyce, geometrii, analizie numerycznej
