...
Omówiê teraz jedn± z najbardziej efektywnych metod wykrywania kolizji pomiêdzy dwoma wielo¶cianami wypuk³ymi. Jest to algorytm Gilbert-Johnson-Keerthi (w skrócie GJK) opublikowany po raz pierwszy w 1988 roku w pracy " Robotics and Automation".  Wyznacza on najmniejsz± odleg³o¶æ pomiêdzy dwoma wypuk³ymi zbiorami i w przeciwieñstwie do wielu innych algorytmów, nie wymaga, aby geometria obiektu by³a przedstawiana w specjalnej formie. 

Algorytm ten jest oparty na metodzie sympleksu. Istot±  algorytmu jest to, i¿ nie operuje on na dwóch obiektach explicite, ale na ich ró¿nicy Minkowskiego:

\begin{df}[Ró¿nica Minkowskiego]
$$A-B = \{a-b: a \in A, b \in B \}.$$
\end{df}

Transformacja taka redukuje problem znalezienia odleg³o¶ci pomiêdzy dwoma wypuk³ymi zbiorami do znalezienia odleg³o¶ci pomiêdzy zbiorem, a punktem. Algorytm opiera siê bowiem na fakcie, ¿e odleg³o¶æ pomiêdzy $A$ i $B$ jest równa odleg³o¶ci pomiêdzy zbiorem bêd±cym ich ró¿nic± Minkowskiego, a pocz±tkiem uk³adu wspó³rzêdnych:
$$dist(A,B) = min \{ ||a - b|| : a \in A, b \in B \} = min \{ ||c|| : c \in A-B\}.$$
\begin{figure}[htbp]
 \centering
    %\includegraphics[width=0.65\textwidth,viewport=50 260 400 1000]{pop10/wykres10.eps}
  \includegraphics[width=1.0\textwidth]{MinkowskiDifference.eps}
\caption[Rozmiar 20]{Odleg³o¶æ pomiêdzy A i B jest równa odleg³o¶ci pomiêdzy ró¿nic± Minkowskiego, a pocz±tkiem uk³adu wspó³rzêdnych}
  \label{zrodlo}
\end{figure}

Kluczowym punktem algorytmu jest to, ¿e ró¿nica Minkowskiego nie jest obliczana explicite. Próbkuje kolejne wyznaczane podzbiory ró¿nicy Minkowskiego, u¿ywaj±c pojêcia funkcji podpieraj±co-rzutuj±cej.

\begin{df}[Funkcja podpieraj±co-rzutuj±ca]
Dla danego obiektu wypuk³ego $A$ jest to funkcja $s_A(d)$, która rzutuje podany kierunek $d$ w kierunku podpieraj±cego punktu $A$ w tym kierunku
\end{df}

Poniewa¿ funkcja rzutuj±ca jest maksimum pewnej funkcj liniowej, to dla ró¿nicy Minkowskiego dostajemy
$$s_(A-B)(d) = s_A(d) - s_B(-d).$$
i zwi±zku z tym mo¿emy to obliczaæ z funkcji rzutuj±cej dla ka¿dego z obiektów $A$ i $B$ osobno.

W celu znalezienia ró¿nicy Minkowskiego $C$ i pocz±tku uk³adu wspó³rzêdnych algorytm GJK wykorzystuje wyniki twierdzenia Caratheodory'ego. Zanim je przedstawiê, wprowadzê definicjê otoczki wypuk³ej:
...
\section{Opis wykorzystanej technologii}
