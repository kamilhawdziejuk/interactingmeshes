...
Omówiê teraz jedn± z najbardziej efektywnych metod wykrywania kolizji pomiêdzy dwoma zbiorami wypuk³ymi. Jest to algorytm Gilbert-Johnson-Keerthi\footnote{Zosta³ opublikowany po raz pierwszy w 1988 roku w pracy Robotics and Automation".}(w skrócie GJK).  Wyznacza on najmniejsz± odleg³o¶æ pomiêdzy dwoma wypuk³ymi zbiorami i w przeciwieñstwie do wielu innych algorytmów, nie wymaga, aby geometria obiektu by³a przedstawiana w specjalnej formie.
...
Poniewa¿ funkcja support-mapping jest maksimum pewnej funkcj liniowej, to dla ró¿nicy Minkowskiego dostajemy
...
W celu znalezienia odleg³o¶ci $A \ominus B$  od pocz±tku uk³adu wspó³rzêdnych algorytm GJK wykorzystuje wyniki twierdzenia Caratheodory'ego. Zanim je przedstawiê, wprowadzê definicjê otoczki wypuk³ej:
...
Opieraj±c siê na powy¿szym twierdzeniu, w algorytmie GJK ogranicza siê przeszukiwania, przechowuj±c w ka¿dej iteracji $k$ zbiór $W_k$, z³o¿ony maksymalnie z $d+1$ punktów z $A \ominus B$. W kolejnej iteracji konstruowany jest sympleks po³o¿ony bli¿ej $O$ ni¿ ten z poprzedniej iteracji. Definiujemy $v_k := v(conv(W_k))$ oraz zbiór $W_k$ nastêpuj±co:
$$W_{k} := W_k \cup v_k.$$


Je¶li pocz±tek uk³adu wspó³rzêdnych zawiera siê w aktualnym sympleksie, to oznacza, ¿e obiekty $A$ i $B$ siê przecinaj± i algorytm siê zatrzymuje. W przeciwnym razie zbiór $W_k$ zostaje zaktualizowany tak, aby tworzy³ nowy sympleks, który zawiera punkt bli¿szy pocz±tku uk³adu wspó³rzêdnych ni¿ aktualny sympleks. 
...
Definiujemy:
\begin{center}
$v_k := v(conv(W_k))$ oraz $W_{k+1} := W_k \cup v_k.$
\end{center}
...
Poniewa¿ $v_k \in conv(W_k)$, wiêc segment $v_kw_k$ zawiera siê w zbiorze $conv(W_k \cup \{w_k\})$. St±d, dla ka¿dego punktu $u$ na segmencie $v_kw_k$ mamy $ ||v_k|| \leq ||u||$. Zgodnie z lematem 4.1, dostajemy
$$||v||^2 - v_k \cdot w_k \leq 0.$$
Z Lematu 4.2 wiemy, ¿e 
