...
namespace InteractingMeshes
{
    ...
    public class ConvexCollisionDetector : ICollisionDetector
    {
        ...
        public Options.CollisionStage CollisionLevel
        {
            get
            {
                return Options.CollisionStage.GJK;
            }
        }
        ...
        public static bool BodiesIntersect(IList<Vector3> shape1, IList<Vector3> shape2)
        {
            ...
            Vector3 S = MaxPointInMinkDiffAlongDir(shape1, shape2, initialPoint); //O(n)
            Vector3 D = -S;
            ...
            for (int i = 0; i <  MaxIterations; i++)
            {
                ...
                if (UpdateSimplexAndDirection(simplex, ref D)) //O(n)
                {
                    return true;
                }
            }
        ...
        /// </summary> 
        private static Vector3 MaxPointInMinkDiffAlongDir(IList<Vector3> shape1, IList<Vector3> shape2,
                                                          Vector3 direction)
        {
            var negateDir = new Vector3(-direction.X, -direction.Y, -direction.Z);
            return MaxPointAlongDirection(shape1, direction) - MaxPointAlongDirection(shape2, negateDir); //O(n)*2
                //Vector3.Negate(direction)); 
        }
